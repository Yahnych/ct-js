generic-menu(class="{opened: opts.menu.opened}" ref="root")
    ...
    script.
        this.onItemClick = e => {
            console.log(e, e.item, e.target, e.target.closest('generic-menu'), this);
            ...
            } else if (e.target.closest('generic-menu') === this.refs.root) { // prevent closing if a label with a submenu was clicked *directly*
                e.stopPropagation();
            }
        };
        ...
        const clickListener = e => {
            ...
            console.log('global click listener', e);
            if (e.target.closest('generic-menu') && e.target.closest('generic-menu') !== this.refs.root) {
